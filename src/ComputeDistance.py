...
def function_to_use_to_compute_EM(data: DataCentralized, i: int, j: int):
    d_iid = compute_EM_distance(data.features_iid[i], data.features_iid[j], stochastic=True)
    d_heter =compute_EM_distance(data.features_heter[i], data.features_heter[j], stochastic=True)
    return d_iid, d_heter
...
#
# def compute_matrix_of_distances_on_decentralized_features(used_distance, data: DataDecentralized, metrics: Metrics) -> Metrics:
#
...
#     return metrics
#
# def compute_matrix_of_distances_on_centralized_features(used_distance, data: DataDecentralized, metrics: Metrics) -> Metrics:
#
#     print("Computing matrix of distances on features ...")
#     # output = getattr(data, attribute_name)
#     # print(output)
...
#     distances_heter = np.zeros((data.nb_of_clients, data.nb_of_clients))
#     for i in tqdm(range(data.nb_of_clients)):
#         for j in range(i, data.nb_of_clients):
#             distances_iid[i,j] = compute_EM_distance(data.features_iid[i], data.features_iid[j], stochastic=True)
#             distances_iid[j,i] = distances_iid[i,j]
#
#             distances_heter[i, j] = compute_EM_distance(data.features_heter[i], data.features_heter[j], stochastic=True)
#             distances_heter[j, i] = distances_heter[i, j]
#     metrics.add_distances(distances_iid, distances_heter)
#     return metrics
#
